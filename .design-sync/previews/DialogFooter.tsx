...
// Pied à deux actions : `flex-col-reverse sm:flex-row sm:justify-end` — au-dessus
// de 640px les boutons s'alignent à droite, l'action principale en dernier.
...
// Pied à action unique pleine largeur : écran de confirmation.
export const ActionUnique = () => (
  <Dialog defaultOpen>
    <DialogContent style={{ ...panel, borderRadius: 8 }}>
      <DialogHeader>
        <p className="font-mono" style={kicker}>Paiement accepté</p>
        <DialogTitle className="font-display font-bold uppercase" style={bigTitle}>
          Table confirmée
        </DialogTitle>
        <DialogDescription className="font-sans" style={bodyText}>
          450 € débités · Sala Mirador, samedi 14 août à 23:00.
        </DialogDescription>
      </DialogHeader>
      <div style={block}>
        <Ligne label="Référence" value="YN-8F42-MAD" />
        <Ligne label="Hôte VIP" value="Lucía" />
      </div>
      <DialogFooter>
        <Button style={{ width: '100%' }}>Voir ma réservation</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
);
