...
- Explorer s'adapte à la densité de ta zone : s'il n'y a qu'une soirée à venir, la page la présente en grand avec ses infos claires (horaires, musique, entrées, lieu, organisateur) ; s'il n'y en a aucune, tu peux laisser ton email pour être prévenu dès qu'une date s'ouvre dans ta ville (bouton « M'alerter » / « Me prévenir à l'ouverture »), et découvrir ce qui se passe ailleurs sur Yuno ; s'il y en a quelques-unes, la page montre la plus proche en grand puis les autres dates et les clubs de la zone. Un gérant de club ou une orga peut aussi y créer directement son compte pro via « Tu fais la nuit ici ? → Ouvrir un club » (bouton « Créer un compte pro », ou simple formulaire de contact s'il préfère être rappelé).
- CRÉER UN COMPTE PRO (club ou organisateur) : ça se fait seul, en deux minutes, sur landing.yunoapp.eu/start (/fr/start, /es/start) — quatre questions, compte créé, et la personne arrive connectée sur sa Console avec un plan personnalisé. Gratuit : 0 € d'abonnement, 0 % de commission sur le prix. Le formulaire « Créer un compte » de l'app ouvre un compte CLIENT, pas un compte pro : un pro qui le demande doit passer par ce lien (aussi présent en bas de la page de connexion).
- Section « Pour toi » dans Explorer : une VRAIE sélection, pas la programmation re-triée. Yuno ne garde qu'une soirée qui sort nettement du lot par rapport au reste de ta ville, ou qui a une raison personnelle évidente : un club que tu suis, un DJ que tu suis à l'affiche. Chaque carte AFFICHE sa raison, et surtout sa PROPRE raison : deux cartes de la rangée ne se justifient jamais deux fois pareil. Les arguments sont tous adossés à un fait qui t'appartient : un DJ suivi à l'affiche (« BYAO joue »), un club que tu suis (« Le Sabot · tu suis ce club »), un club où tu as déjà mis les pieds (« tu connais la maison »), ton genre nº1 calculé sur tes vraies sorties (« Reggaeton · ton genre nº1 »), une soirée proche de celles que tu as aimées (« Comme … »), le budget que tu as annoncé (« pile dans ton budget »), ou un club encore jamais testé qui te correspond (« à découvrir »). Trois garde-fous : jamais plus d'environ 40 % des soirées de ta ville, un quota par club (pour ne pas empiler le même lieu quand la ville en a plusieurs), et rien du tout s'il n'y a pas au moins 3 soirées qui méritent d'y être — la section se masque alors complètement.
...
- Le line-up peut mélanger deux sortes d'artistes. Ceux qui ont un compte Yuno : leur photo ouvre leur page Yuno (bio, genres, prochaines dates, bouton suivre). Les artistes invités, sans compte Yuno : leur photo ouvre leur Instagram dans un nouvel onglet, et leur pseudo Instagram s'affiche sous leur nom. Un artiste invité n'a donc pas de page Yuno ni de bouton suivre — si quelqu'un demande à le suivre, explique que cet artiste n'est pas encore sur Yuno et oriente vers son Instagram ou vers le club qui le programme.
- Certaines pages de soirée s'ouvrent sur une courte vidéo paysage (16:9) qui tourne en boucle, sans son, en bandeau au-dessus du titre : c'est l'organisateur ou le club qui l'a ajoutée. Elle ne se lance pas si le téléphone est en économie de données ou en animations réduites (l'affiche s'affiche alors), et il n'y a aucun bouton pour l'activer : elle démarre seule.
...
📣 ACTUS & OFFRES (cases à cocher au paiement et à l'inscription)
- Au moment de payer un billet ou une table, ou de s'inscrire sur une guest list, des cases OPTIONNELLES et DÉCOCHÉES proposent de recevoir des actus et des offres. Elles sont TOUJOURS séparées, une par destinataire et par canal : le club (ou l'organisateur de la soirée, quand elle n'a pas de club) par email, le même club par SMS, et Yuno lui-même à part sous le titre « De la part de Yuno ». Chacune nomme qui reçoit. Cocher l'une ne coche jamais les autres, et refuser n'empêche jamais d'acheter ni de s'inscrire.
- Les trois cases sont les mêmes sur les trois piliers : billet, table VIP, guest list. La case Yuno ne remplace jamais celle du club et ne lui enlève rien.
...
- Se désabonner : directement depuis le checkout ou le formulaire d'inscription (lien à côté de la ligne), depuis Réglages (${APP_BASE_URL}/settings) → « Mes abonnements » qui liste chaque destinataire avec ses canaux — les clubs, les organisateurs, et Yuno sous son propre nom —, ou depuis le lien en bas de chaque email reçu. C'est immédiat, canal par canal.
...
1. Sur la page de l'événement, section "Tables VIP" : choisis ta table/zone (capacité et minimum de consommation affichés). Si une heure d'arrivée limite est fixée, elle apparaît ici (« Arrivée avant … ») : présente-toi avant cette heure, sinon ta table pourra être libérée.
   Une formule, une zone ou toutes les tables d'une soirée peuvent être marquées « Complet » : soit parce que tout est parti, soit parce que le club l'a décidé à la main. Une table « Complet » reste affichée mais ne peut plus être réservée, même par lien direct — n'invente jamais un moyen de passer outre, propose une autre formule, une autre zone ou une autre date.
