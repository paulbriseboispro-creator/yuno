...
const HELP_ARTICLES: Record<string, { title: string; keywords: string[]; path: string; snippet: string }> = {
  ...
  "external-events": {
    ...
    keywords: ["soirée externe", "external event", "billetterie externe", "récurrence", "recurring", "modèle", "template", "brouillon", "draft", "semaine", "week", "publier", "publish", "genres", "prix", "tables", "guest list", "lien permanent"],
    path: "/affiliate/events",
    snippet: "Une soirée externe (club, date, genres, prix, lien billetterie) publiée apparaît sur le linktree, la page RP et l'agenda public ; le clic vers la billetterie passe par l'interstitiel et compte dans le trafic. Les soirées hebdomadaires se créent une fois en modèle récurrent qui matérialise dix dates d'avance, la vue Semaine montre le planning. Sans lien billetterie une soirée reste en brouillon (invisible) et se publie dès que le lien est posé ; un lien de modèle ne vaut que pour l'édition de sa semaine sauf s'il est marqué permanent. La soirée peut afficher son offre en badges publics : tables VIP, uniquement des tables (clubs qui ne donnent que la vente de tables), et guest list mixte ou femmes. Cocher « uniquement des tables » change aussi le prix affiché partout sur Yuno : la soirée annonce « Tables uniquement » au lieu d'un montant, n'apparaît jamais dans le filtre « gratuit », et ne remonte pas en tête d'un tri par prix croissant. Il est donc inutile de mettre un prix d'entrée sur ce type de soirée. Modifier un modèle (affiche, nom, prix, horaires, genres, tables, guest list) applique d'office le changement à toutes les prochaines dates PAS ENCORE PUBLIÉES ; les dates déjà en ligne gardent leur version, et Yuno demande seulement à l'enregistrement si elles doivent suivre aussi. Le report n'écrit que les champs modifiés — une soirée retouchée à la main garde le reste — et ne touche jamais les liens billetterie ni le statut : une soirée en ligne le reste. Passer un modèle en Inactif met la série en pause : plus aucune nouvelle date, ses brouillons sont masqués (liste, vue Semaine, liens à poser, publieur du modèle) et Yuno propose de retirer de l'app les dates encore en ligne ; le réactiver les rend et propose de remettre en ligne celles qui ont déjà leur lien. Le trafic « apporté par Yuno » compte les visites arrivées depuis une page Yuno (Explore, recherche, carte, app native), et les clics de l'agence elle-même ne sont jamais comptés.",
  },
  "traffic": {
