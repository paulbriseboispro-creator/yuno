...
serve(async (req) => {
  ...
  try {
    ...
    // ({ tables: { enabled: false } }). A pillar out of scope must not sell until
    // an amendment brings it back in.
    if (isCoEventForGuard && isPillarDisabled(event.revenue_split_rules as Record<string, unknown> | null, "table")) {
      logStep("Checkout refused — tables pillar excluded by collab contract", { eventId: event.id });
