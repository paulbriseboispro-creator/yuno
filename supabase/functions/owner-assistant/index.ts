...
const HELP_ARTICLES: Record<string, { title: string; keywords: string[]; path: string; snippet: string }> = {
  ...
  "email-automations": {
    ...
    keywords: ["automatisation", "automatisations", "automation", "automations", "recette", "recettes", "email automatique", "emails automatiques", "panier abandonné", "panier abandonne", "abandoned checkout", "checkout abandonné", "dernier appel", "last call", "merci d'être venu", "merci d'etre venu", "on t'a manqué", "on t'a manque", "pas venu", "no-show", "bienvenue", "welcome", "reconquête", "reconquete", "win-back", "winback", "client dormant", "inactifs", "non-ouvreurs", "non ouvreurs", "renvoyer", "renvoi", "resend", "pas ouvert", "n'ont pas ouvert", "meilleure heure", "heure d'envoi", "quelle heure envoyer", "best hour", "envoi automatique", "relance automatique", "partent seuls", "flow", "flows", "parcours", "règles yuno", "regles yuno", "pression", "trop d'emails", "fatigue", "protégés", "proteges", "anti-spam", "désabonnements", "spam"],
    path: "/owner/campaigns/automations",
    snippet: "AUTOMATISATIONS EMAIL (Campagnes email → Automatisations, ou l'entrée « Automatisations email » du menu) : six recettes qui partent SEULES, dans le design du club, par le même moteur que les campagnes (quota, rodage, liste de suppression, pas d'envoi la nuit, liens suivis, revenu attribué par pilier). Chaque recette = un interrupteur + un délai + un modèle Email Studio + un objet, et un bilan (en file, envoyés, ouvertures, clics, revenu attribué, désabonnés, exclus avec leur raison). Les six : (1) PANIER ABANDONNÉ — billet ou table lancés au paiement et jamais payés, 1/2/4/12 h après, seulement si la personne a coché l'accord marketing au paiement (l'accord est versé dans la base avant l'envoi) ; c'est l'email au meilleur revenu de tout le marché. (2) DERNIER APPEL — 12/24/48/72 h avant chaque soirée publiée qui vend encore (billets, tables ou liste invités non marqués complets), à toute la base opt-in qui n'a ni billet, ni table, ni place en liste invités. (3) MERCI D'ÊTRE VENU — 6 à 48 h après la fin, à ceux dont le QR a été SCANNÉ à la porte, avec la prochaine soirée publiée en carte. (4) ON T'A MANQUÉ — 12 à 72 h après la fin, à ceux qui avaient une place (billet, table, liste invités) et n'ont pas été scannés. Ces deux recettes EXIGENT le scan : sans aucun scan sur la soirée, rien ne part. (5) BIENVENUE — 1 à 48 h après une nouvelle inscription dans la base (liste invités, formulaire, suivi), jamais pour un fichier importé ni pour un acheteur, et seulement pour les inscriptions postérieures à l'activation. (6) RECONQUÊTE — aux clients dont la dernière venue remonte à 45/60/90/120 jours, une fois par semestre, 300 personnes max par passage. Allumer une recette sans modèle crée automatiquement le modèle Yuno (retouchable dans le studio, bouton « Modifier ce modèle dans le studio ») ; « Aperçu et test » montre l'email tel qu'il partira avec la prochaine soirée et l'envoie à l'adresse du pro. RÈGLES YUNO (non modifiables, tous expéditeurs confondus — clubs, organisateurs et Yuno lui-même, parce que la réputation d'envoi est partagée) : une personne ne reçoit pas plus d'une automatisation par 24 h ni trois par semaine (le panier abandonné a un peu plus de marge : 2/24 h, 5/semaine) ; une campagne manuelle ne dépasse pas 3/jour et 8/semaine ; 8 emails marketing en 90 jours sans une seule ouverture = la personne « dort » jusqu'à sa prochaine ouverture ; deux désabonnements récents chez deux expéditeurs = plus aucune automatisation pendant 30 jours ; une soirée ne déclenche qu'UN dernier appel, UN merci, UN « on t'a manqué » par personne quel que soit l'expéditeur, et les recettes du pro passent avant celles de Yuno. L'effet se lit dans le rapport de campagne (« protégés par les règles Yuno ») et dans le bilan de chaque recette (exclus « protégés », « endormis », « en retrait »). Ce n'est pas une perte : ces personnes partaient déjà. Yuno a retiré ses anciens emails « ta soirée en chiffres » et l'upsell post-achat ; il ne garde que « on t'a manqué » et les recommandations, eux aussi soumis aux règles. GARANTIES : personne ne reçoit une automatisation sans être dans la base opt-in ; acheteurs, inscrits, désabonnés et injoignables exclus AU MOMENT de l'envoi ; au plus UNE automatisation par personne et par 48 h (sauf panier abandonné) ; jamais entre 23 h et 9 h ; un email sans soirée à relier perd ses blocs Yuno (jamais de tarifs inventés). Le moteur passe toutes les 5 minutes ; chaque envoi automatique a son rapport complet comme une campagne. L'assistant peut lister les recettes (list_email_automations) et en allumer/éteindre une ou changer son délai (set_email_automation, avec confirmation) — mais il ne peut pas créer le modèle : sans modèle, le pro doit ouvrir la page et cliquer « Créer le modèle Yuno ». RENVOYER AUX NON-OUVREURS (écran Planification, ou rapport d'une campagne partie) : le même email, sous un autre objet, 24/48/72 h après la fin de l'envoi, à ceux qui ne l'ont pas ouvert — +20 à 30 % de lecture sans rien réécrire. Jamais à ceux qui ont ouvert/cliqué, ni aux désabonnés/injoignables, ni aux acheteurs ou inscrits de la soirée entre-temps, ni après le début, ni la nuit ; un seul renvoi par campagne ; bilan dans le rapport de la campagne mère ; la relance après clic compte aussi les clics du renvoi. MEILLEURE HEURE DE LA BASE (écran Planification) : ouvertures des 120 derniers jours par heure et par jour (heure de Paris) ; sous 30 ouvertures rien n'est conclu, au-delà la carte nomme la fenêtre de 2 h la plus ouverte et le meilleur jour.",
  },
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "List the club's automatic email recipes (abandoned_checkout, last_call, post_event_thanks, post_event_missed, welcome, win_back): enabled or not, delay, template attached, and their report (queued, sent, opens, clicks, skipped with reasons). Use for any question about automatic emails, 'relance automatique', 'panier abandonné', 'dernier appel', 'merci', 'on t'a manqué', 'bienvenue', 'reconquête'.",
      parameters: { type: "object", properties: {}, required: [] },
    },
  },
  {
    ...
    function: {
      ...
      description: "Enable/disable an automatic email recipe or change its delay. WRITE action — requires confirmation. Enabling requires a template already attached (created from the Automations page); if none, tell the owner to open /owner/campaigns/automations and click « Créer le modèle Yuno ».",
      parameters: {
        type: "object",
        properties: {
          kind: { type: "string", enum: ["abandoned_checkout", "last_call", "post_event_thanks", "post_event_missed", "welcome", "win_back"], description: "The recipe" },
          enabled: { type: "boolean", description: "true to enable, false to disable" },
          delay_hours: { type: "integer", description: "Optional delay in hours (win_back in hours too: 90 days = 2160). abandoned_checkout 1-12, last_call 12-72 before start, post_event 6-72 after end, welcome 1-48, win_back 1080-2880." },
        },
        required: ["kind"],
      },
    },
  ...
  try {
    switch (toolName) {
      ...
      case "list_email_automations": {
        ...
        const KINDS = ["abandoned_checkout", "last_call", "post_event_thanks", "post_event_missed", "welcome", "win_back"];
        const byKind: Record<string, any> = {};
        for (const r of rows) byKind[r.kind] = r;
        const out = KINDS.map((kind) => {
          const r = byKind[kind];
          if (!r) return { kind, configured: false, enabled: false };
          return {
            kind, configured: true, enabled: !!r.enabled, delay_hours: r.delay_hours,
            has_template: !!r.template_id, subject_override: r.subject || null,
            report: { pending: r.pending, queued: r.queued, sent: r.sent, opens: r.opens, clickers: r.clickers, unsubscribes: r.unsubscribes, campaigns: r.campaigns, skipped: r.skipped },
          };
        });
        return JSON.stringify({ automations: out, page: "/owner/campaigns/automations", note: "A recipe without template sends nothing: the owner creates it from the Automations page (« Créer le modèle Yuno »). Delays: abandoned_checkout hours after checkout, last_call hours BEFORE start, post_event_* hours after end, welcome hours after signup, win_back hours since last visit (2160 = 90 days)." });
      }
      case "set_email_automation": {
        ...
        if (Number.isFinite(Number(args.delay_hours)) && Number(args.delay_hours) > 0) patch.delay_hours = Math.min(8760, Math.max(1, Math.round(Number(args.delay_hours))));
