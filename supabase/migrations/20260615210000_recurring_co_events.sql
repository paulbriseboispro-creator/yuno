-- ============================================================================
-- Soirées récurrentes en CO-EVENT (club <-> organisateur).
-- Avant : owner_recurring_templates ne pouvait générer que des events solo
-- (venue_id XOR organizer_user_id). Ajout d'un partenaire optionnel : quand
-- `partner_organizer_id` est défini sur un template VENUE, les occurrences
-- générées deviennent des co-events (partner_organizer_id + event_mode='co_event'
-- + revenue_split_rules + split approuvé), donc visibles côté club (Collaborations)
-- ET côté organisateur (ses events / analytics via partner_organizer_id).
-- ============================================================================

ALTER TABLE public.owner_recurring_templates
  ADD COLUMN IF NOT EXISTS partner_organizer_id uuid,
