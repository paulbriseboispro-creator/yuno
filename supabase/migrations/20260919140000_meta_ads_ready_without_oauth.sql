-- Meta — la publicité ne dépend plus du MODE de connexion, mais des ACTIFS.
--
-- `ads_ready` exigeait `mode = 'oauth'`, donc Facebook Login for Business.
-- Or Meta authentifie là un PROFIL Facebook, et un compte professionnel ouvert
-- depuis Instagram n'en a pas : ces pros étaient définitivement privés de la
-- page Publicité, même en collant un jeton parfaitement habilité.
--
-- Un jeton d'utilisateur système (Business Manager → Paramètres → Utilisateurs
-- → Utilisateurs système) porte `ads_management`, n'expire pas, et se crée SANS
-- Facebook Login. `meta-connect` action `save` découvre désormais ce qu'un
-- jeton collé sait faire et remplit `ad_account_id` / `page_id` / `ig_user_id`
-- comme le ferait le retour OAuth. La bonne question devient donc : ce compte
-- a-t-il un compte publicitaire et une Page ? Le mode n'y change rien.
--
-- La garde d'autorisation ne bouge pas : `meta_scope_allowed` reste la porte,
-- et le `search_path` est toujours figé.
