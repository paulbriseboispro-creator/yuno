-- ─── Tour de contrôle : première passe de corrections (appliquée telle quelle) ──
-- • admin_ai_usage : p95 de latence sorti dans un CTE.
-- • admin_cockpit / admin_release_health : un seul balayage matérialisé de
--   l'historique pg_cron sur 7 jours.
-- Voir 20260910141000 pour la suite (by_day de l'IA, cockpit sans pg_cron).
...
BEGIN
  ...
  cron_recent AS MATERIALIZED (
    -- Un seul balayage de l'historique pg_cron (80 000+ lignes, sans index).
    SELECT r.jobid, r.status, r.start_time FROM cron.job_run_details r WHERE r.start_time >= v_7d
  ),
  pulse AS (
    SELECT
      jsonb_build_object(
  ...
  SELECT jsonb_build_object(
    ...
    'health', jsonb_build_object(
      ...
      'crons_failed_24h', (SELECT count(*) FROM cron_recent r WHERE r.start_time >= now() - interval '24 hours' AND r.status <> 'succeeded'),
      'crons_failed_7d', (SELECT count(*) FROM cron_recent r WHERE r.status <> 'succeeded'),
      'crons_total', (SELECT count(*) FROM cron.job j WHERE j.active),
      'crons_last_run', (SELECT max(r.start_time) FROM cron_recent r),
