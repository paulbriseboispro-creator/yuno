-- Commission promoteur : reconnaître le videur d'un ORGANISATEUR.
--
-- `record_promoter_conversion` autorisait le staff d'un CLUB (is_venue_staff) et
-- l'équipe d'un organisateur (is_org_member), mais pas le staff opérationnel
-- d'un organisateur — celui qui tient justement la porte d'une soirée sans club.
--
-- L'appel étant volontairement fire-and-forget côté porte (une commission qui
-- rate ne doit pas bloquer une entrée), le refus était SILENCIEUX : l'invité
-- entrait, et le promoteur qui l'avait placé n'était jamais payé. Exactement le
-- bug que le commentaire de Bouncer.tsx décrit pour le cas club, transposé aux
-- soirées d'organisateur.
--
-- Corps repris À L'IDENTIQUE de la définition en production (pg_get_functiondef),
-- seul le bloc d'autorisation gagne un terme.

CREATE OR REPLACE FUNCTION public.record_promoter_conversion(p_promoter_id uuid, p_conversion_type text, p_amount numeric, p_event_id uuid DEFAULT NULL::uuid, p_ticket_id uuid DEFAULT NULL::uuid, p_table_reservation_id uuid DEFAULT NULL::uuid, p_order_id uuid DEFAULT NULL::uuid, p_guest_list_entry_id uuid DEFAULT NULL::uuid, p_scan_at timestamp with time zone DEFAULT now(), p_discount numeric DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
      ...
      OR (p_event_id IS NOT NULL AND EXISTS (
        ...
        WHERE e.id = p_event_id
          AND (
            ...
            -- Le videur recruté par l'organisateur tient la porte : c'est LUI
            -- qui scanne l'invité, donc lui qui déclenche la commission. Sans
            -- ce terme, l'appel échoue en silence (il est fire-and-forget) et
            -- le promoteur n'est jamais payé sur les invités qu'il a placés.
            OR public.is_event_door_staff(auth.uid(), e.id)
          )
      ))
    ) THEN
