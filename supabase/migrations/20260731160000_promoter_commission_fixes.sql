-- ─────────────────────────────────────────────────────────────────────────────
-- Corrections du moteur de commission promoteur (audit 2026-07-31).
--
-- Quatre défauts corrigés dans record_promoter_conversion, tous vérifiés à
-- l'audit :
--
--   1. REMISE non stockée / base incohérente. La conversion ne gardait aucune
--      trace de la remise consentie par le promoteur → « X€ de remises données »
--      incalculable. Nouvelle colonne promoter_conversions.discount_amount +
--      paramètre p_discount. La base de commission (p_amount) est désormais le
--      BRUT (valeur faciale, avant remise promoteur) côté billet ET table — les
--      billets étaient déjà en brut, les tables passaient le net ; on aligne les
--      tables sur les billets (jamais de baisse de commission). Les appelants
--      passent le brut + la remise.
--
--   2. Guest list payée 0 sous une fenêtre horaire en POURCENTAGE. Un invité
--      gratuit a un montant nul : ROUND(0 * pct) = 0. La guest list est
--      désormais TOUJOURS payée par tête (euros), et n'entre jamais dans la
--      logique paliers/fenêtres (mécaniques billet).
--
--   3. Paliers (tiers) / fenêtres appliqués au mauvais type. Une table sous un
--      template à paliers touchait la valeur de palier BILLET ; une table sous
--      un template billet-only touchait 0. Le calcul est réécrit en dispatch
--      PAR TYPE : table → règle 'table' du template, à défaut colonnes plates du
--      promoteur (jamais 0 par défaut) ; guest list → par tête ; billet →
--      paliers puis fenêtres puis règle plate.
--
--   4. Progression paliers + bonus en LIFETIME au lieu de PAR SOIRÉE. Le
--      compteur de base (v_base_count) ne filtrait pas l'événement : un club
--      voyait ses paliers avancer avec les ventes faites ailleurs, et le bonus
--      unique ne se déclenchait qu'une fois dans la vie du promoteur. L'UI, elle,
--      affiche un palier event-scopé. On scope le compteur à l'événement.
--
-- Inchangé (et volontairement) : garde d'autorisation, index uniques partiels +
-- ON CONFLICT DO NOTHING (anti-double-compte), plafonds agence, override chef
-- d'équipe, marge agence en AFTER INSERT.
-- ─────────────────────────────────────────────────────────────────────────────

ALTER TABLE public.promoter_conversions
  ADD COLUMN IF NOT EXISTS discount_amount NUMERIC NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.promoter_conversions.discount_amount IS
  'Remise promoteur consentie à l''acheteur sur cette vente (euros). Rapport « remises données ». La commission se calcule sur amount (BRUT), pas sur amount - discount_amount.';

-- Signature élargie (p_discount ajouté) → nouvelle liste d'arguments : on DROP
-- l'ancienne avant de recréer, sinon Postgres garde deux surcharges ambiguës.
DROP FUNCTION IF EXISTS public.record_promoter_conversion(uuid, text, numeric, uuid, uuid, uuid, uuid, uuid, timestamptz);
...
BEGIN
  ...
  -- paliers et au bonus. Scopé À LA SOIRÉE (p_event_id) : un palier « vends 10 »
  -- se compte pour CETTE soirée, pas sur toute la vie du promoteur — sinon les
  -- ventes d'un autre club faisaient avancer le palier ici et le bonus ne
  -- tombait qu'une fois pour toujours. La conversion courante sera la
  -- (v_base_count + 1)-ième de la soirée.
  ...
  IF v_rules IS NOT NULL THEN
    ...
    -- ── Dispatch PAR TYPE. Paliers et fenêtres horaires sont des mécaniques
    --    BILLET (volume, heure d'arrivée) : une table ou une guest list n'y
    --    entrent jamais.
    IF p_conversion_type = 'table' THEN
      ...
      IF v_table_rule IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Template sans règle 'table' → colonnes plates du promoteur (jamais 0
        -- par défaut, l'ancien code laissait la table à 0 sous un template
        -- billet-only).
        SELECT CASE WHEN table_commission_type = 'percentage'
          THEN ROUND(p_amount * (table_commission_value / 100), 2)
          ELSE COALESCE(table_commission_value, 0) END
          INTO v_commission FROM promoters WHERE id = p_promoter_id;
      END IF;

    ELSIF p_conversion_type = 'guestlist' THEN
      -- Guest list : TOUJOURS par tête (euros). Jamais un pourcentage — une
      -- entrée gratuite a un montant nul, donc un % (palier OU fenêtre) donnait
      -- 0 et le promoteur n'était jamais payé sur ses invités.
      v_commission := public.promoter_guestlist_head_commission(v_rules, p_guest_list_entry_id);

    ELSE
      -- Billet (et tout autre type payant) : paliers → fenêtres → règle plate.
    ...
    -- Bonus unique au franchissement du seuil de ventes de la soirée
    -- (rules.bonus). Réservé aux billets : c'est une mécanique de volume billet,
    -- une table ou une guest list ne doit pas déclencher le bonus billet.
...
-- SÉCURITÉ : un DROP + CREATE repart sur le défaut Postgres (EXECUTE à PUBLIC).
-- Or la garde d'autorisation est SAUTÉE quand auth.uid() est NULL (chemin
-- service-role). Un appelant anon aurait donc pu forger n'importe quelle
-- conversion. On révoque PUBLIC/anon et on ne rouvre qu'à authenticated (les
-- scanners) + service_role (les edge functions).
REVOKE ALL ON FUNCTION public.record_promoter_conversion(uuid, text, numeric, uuid, uuid, uuid, uuid, uuid, timestamptz, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_promoter_conversion(uuid, text, numeric, uuid, uuid, uuid, uuid, uuid, timestamptz, numeric) TO authenticated, service_role;
