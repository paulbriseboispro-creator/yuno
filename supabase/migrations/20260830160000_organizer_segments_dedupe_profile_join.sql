-- Fichier client organisateur : une ligne par personne, jamais deux.
--
-- `LEFT JOIN profiles p ON lower(p.email) = ag.em` supposait un profil unique
-- par email. Ce n'est pas vrai : comptes orphelins et comptes vitrine peuvent
-- partager une adresse (5 cas en base). Chaque doublon de profil dupliquait la
-- ligne client de la personne.
--
-- Le défaut était latent tant que le fichier client ne contenait que des
-- acheteurs de billets ; il devient visible maintenant que les inscrits guest
-- list y figurent. On ne retient qu'un profil : celui qui a réellement fait
-- l'activité (ident.user_id), sinon le plus récemment créé.
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT pr.id, pr.first_name, pr.last_name, pr.phone
    FROM profiles pr
