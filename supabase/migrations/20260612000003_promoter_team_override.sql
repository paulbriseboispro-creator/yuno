-- Phase 4 — Override agence (base).
-- Une équipe peut définir un override : une part de la commission du promoteur
-- terrain est reversée à son chef d'équipe (manager d'agence). Ex : 4€/tête, dont
-- 1€ d'override → 3€ au promoteur, 1€ au chef. Calculé au scan, dans la même
-- transaction que la commission du promoteur.

-- 1. Config override sur l'équipe
...
-- 2. Traçabilité de l'override sur la conversion
ALTER TABLE public.promoter_conversions
  ADD COLUMN IF NOT EXISTS parent_conversion_id uuid REFERENCES public.promoter_conversions(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS override_amount numeric NOT NULL DEFAULT 0;

-- 3. Autoriser le type 'override' (ligne créditée au chef d'équipe)
ALTER TABLE public.promoter_conversions
...
ALTER TABLE public.promoter_conversions
  ...
  CHECK (conversion_type IN ('order', 'ticket', 'table', 'guestlist', 'override'));

-- 4. record_promoter_conversion : ajoute le split override après l'attribution.
--    Signature identique (9 args) → simple CREATE OR REPLACE.
