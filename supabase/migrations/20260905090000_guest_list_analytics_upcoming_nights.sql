-- Analytics guest list : les soirées À VENIR comptent.
--
-- La fenêtre p_from/p_to sélectionnait les soirées par date (events.start_at),
-- donc une liste dont la soirée est la semaine prochaine n'apparaissait nulle
-- part en vue « 7 jours » alors que ses inscrits arrivent maintenant — la zone
-- « La soirée » (sans fenêtre) affichait 0/2 et la zone Guest list « pas encore
-- d'inscrit ». Club et organisateur avaient le même angle mort.
--
-- Règle : une liste entre dans la période si sa soirée y tombe OU si elle a reçu
-- une inscription pendant la période. Une soirée passée garde tous ses inscrits
-- (même ceux inscrits avant la fenêtre) ; une soirée à venir apparaît dès la
-- première inscription. Même signature, CREATE OR REPLACE suffit.
