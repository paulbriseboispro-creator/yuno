-- ─────────────────────────────────────────────────────────────────────────────
-- Automatisations email v2 — trois recettes de plus.
--
--   · table_upsell  « Passe en table » : N h avant le début, aux détenteurs
--                   d'un billet payé sans table, tant qu'il reste une table.
--   · tier_closing  « Le tarif monte » : le palier ouvert dépasse un seuil
--                   (75 / 85 / 95 %), il reste un billet et un palier plus
--                   cher suit ; à ceux qui ont montré un intérêt sans acheter
--                   (clic sur la soirée dans un email, liste d'attente).
--                   Recette URGENTE : palier de pression 2 / 24 h, 5 / 7 j,
--                   exemptée du cooldown de 48 h comme le panier abandonné.
--   · new_event     « Nouvelle soirée » : N h après la PUBLICATION
--                   (events.published_at, jamais une re-génération de modèle
--                   récurrent), à toute la base opt-in — fichiers importés
--                   compris. Rafale : six dates publiées d'un coup n'annoncent
--                   que la plus proche, les autres sont tracées.
--
-- Même mécanique que les six premières (20260915120000 + 140000) : un kind,
-- une branche « candidats », le jugement au moment dû, une campagne enfant
-- par (recette, soirée). Rien ne part hors du registre de consentement.
-- ─────────────────────────────────────────────────────────────────────────────

-- ── 1. Les kinds, le seuil de palier, les nouvelles raisons ─────────────────
...
-- Le palier n'est pas un délai, c'est un seuil : colonne propre, delay_hours
-- reste à sa valeur par défaut et l'écran masque le sélecteur.
ALTER TABLE public.email_automations
  ADD COLUMN IF NOT EXISTS threshold_pct integer NOT NULL DEFAULT 85;
ALTER TABLE public.email_automations DROP CONSTRAINT IF EXISTS email_automations_threshold_check;
ALTER TABLE public.email_automations ADD CONSTRAINT email_automations_threshold_check
  CHECK (threshold_pct IN (75, 85, 95));
COMMENT ON COLUMN public.email_automations.threshold_pct IS
  'tier_closing : part du palier ouvert vendue (75 / 85 / 95 %) à partir de laquelle « le tarif monte » part.';
...
-- ── 1b. Vocabulaire des statuts : events.status = 'active' ───────────────────
-- Le moteur v1 (20260915120000) filtrait sur ('published', 'featured'), le
-- vocabulaire d'affiliate_events : aucune soirée Yuno ne le porte (status =
-- active | cancelled | postponed), donc dernier appel, merci, on t'a manqué et
-- « prochaine soirée » ne trouvaient RIEN. Corrigé ici, partout (moteur, aperçu,
-- prochaine soirée).
...
-- ── 2. Tables restantes d'une soirée ─────────────────────────────────────────
-- Même calcul que le bloc Table VIP de l'email (fetchStudioLiveData) :
-- formules actives de la soirée (ou du club quand la formule n'est pas
-- event-scopée) moins réservations payées / confirmées, formules marquées
-- complètes exclues. NULL = la soirée n'ouvre aucune table.
...
$$;
REVOKE ALL ON FUNCTION public._event_tables_left(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._event_tables_left(uuid) TO service_role, authenticated;

-- ── 3. Politique : « le tarif monte » est une rareté réelle → palier urgent ──
...
COMMENT ON FUNCTION public.email_send_policy(text, text) IS
  'Règles Yuno non modifiables : suppression, pression (auto 1/24h·3/7j, urgent [panier abandonné, tarif qui monte] 2/24h·5/7j, campagne 3/24h·8/7j), fatigue (8 envois/90 j sans ouverture), aversion (2 désabonnements/30 j).';

-- ── 4. Le moteur, avec les trois recettes ───────────────────────────────────
