-- ─────────────────────────────────────────────────────────────────────────────
-- Audience Intelligence — Phase 2 : segmentation.
--
-- get_audience_segments(subject) : agrégats de segments SANS PII.
--   • engagement : engaged (a cliqué un push < 90 j) / passive (joignable, pas de
--     clic) / unreachable (pas de token push).
--   • superfans  : notify_all (DJ « je suis partout »).
--   • cohorte    : nouveaux < 30 j / établis.
--   • venue only : convertis (abonnés ∩ venue_customers) + paliers de valeur
--     (bronze/silver/gold) + récence (active/at_risk/lapsed).
-- Réutilise audience_members + can_read_audience (Phase 1).
-- ─────────────────────────────────────────────────────────────────────────────
