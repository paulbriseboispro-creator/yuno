-- =============================================================================
-- ARRÊT TOTAL DE LA GÉNÉRATION DES CO-SOIRÉES : ON CONFLICT ne sait pas inférer
-- un index unique PARTIEL.
--
-- 20260720240000 a remplacé, pour rendre une proposition refusée re-proposable :
--     event_id uuid NOT NULL UNIQUE
--   par
--     CREATE UNIQUE INDEX event_collab_contracts_live_event_idx
--       ON public.event_collab_contracts (event_id) WHERE status <> 'cancelled';
--
-- Substitution correcte côté métier, mais generate_recurring_events insère son
-- contrat d'occurrence avec `ON CONFLICT (event_id) DO NOTHING`. Postgres n'infère
-- un index PARTIEL que si la clause ON CONFLICT répète son prédicat ; sans lui il
-- lève « there is no unique or exclusion constraint matching the ON CONFLICT
-- specification ».
--
-- Et cette erreur est AVALÉE : l'insertion du contrat vit dans le bloc
-- BEGIN … EXCEPTION WHEN OTHERS de la boucle par date. La sous-transaction est
-- donc annulée EN ENTIER — y compris l'INSERT de l'event lui-même, exécuté juste
-- avant. Depuis 20260720240000, PLUS AUCUNE occurrence de co-soirée récurrente
-- n'est créée, pour aucun club, et la seule trace est un RAISE WARNING dans les
-- logs Postgres. Les séries solo, elles, continuent : le bloc fautif est sous
-- `IF v_is_co`. D'où un silence total côté produit.
--
-- Constaté au rattrapage de 20260721100000, qui a émis exactement ce warning sur
-- « Amore Night » (2026-07-22) et « Yuno Electronic Body » (2026-07-24) : les
-- deux séries co-organisées de womber, l'une bloquée à l'adoption, l'autre à la
-- création. La correction est d'inférer explicitement l'index partiel :
--     ON CONFLICT (event_id) WHERE status <> 'cancelled' DO NOTHING
--
-- Le reste de cette fonction est identique à 20260721100000, dont l'objet est
-- rappelé ci-dessous — son rattrapage n'a rien pu adopter, il est rejoué ici.
--
-- -----------------------------------------------------------------------------
-- (20260721100000) Adopter les occurrences DÉJÀ générées quand une série solo
-- devient une collab.
--
