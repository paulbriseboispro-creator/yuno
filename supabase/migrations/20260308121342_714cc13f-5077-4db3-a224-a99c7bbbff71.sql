...
-- 1. Add FK with CASCADE on events.venue_id
ALTER TABLE public.events
ADD CONSTRAINT events_venue_id_fkey
FOREIGN KEY (venue_id) REFERENCES public.venues(id) ON DELETE CASCADE;

-- 2. Update admin_delete_venue to explicitly delete events (and their dependents)
...
BEGIN
  ...
  -- Explicitly delete events (cascades to event_djs, event_organizers, etc.)
