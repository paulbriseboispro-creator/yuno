-- Analytics guest list : le no-show se juge À LA FERMETURE de la soirée.
--
-- Une inscription sur une soirée à venir sortait en no-show : « 2 inscrits,
-- 0 venus, no-show 100 % » alors que la soirée n'a simplement pas encore eu
-- lieu. Le chiffre qui devait dire « ces gens n'étaient pas là » disait « cette
-- soirée est dans trois jours ». Un club ne peut rien décider avec ça, et ça
-- pollue toutes les moyennes (par propriétaire de liste, par type, par genre).
--
-- Trois états d'une soirée, un seul compte pour le no-show :
--   • à venir   — start_at dans le futur : l'invité est en attente, jamais un no-show
--   • en cours  — la porte est ouverte : la présence se lit en direct, le no-show
--                 n'est pas encore arrêté (les gens arrivent toute la nuit)
--   • terminée  — end_at (ou start_at + 8 h) + 2 h de grâce : le no-show est figé
--
-- Les 2 h de grâce sont celles utilisées partout ailleurs pour « la soirée est
-- finie » (crédits conso, MyOrders). Conséquences :
--   • no_show / no_show_rate         → soirées TERMINÉES uniquement
--   • show_rate                      → soirées OUVERTES (en cours + terminées)
--   • upcoming                       → nouveau compteur, les inscrits en attente
