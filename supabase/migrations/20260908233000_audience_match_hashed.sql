-- ───────────────────────────────────────────────────────────────────────────
-- Croiser des segments, en jointure hachée.
--
-- 20260908232000 filtrait `cseg` par segment dans une sous-requête corrélée
-- à chaque abonné : 12 000 abonnés × re-résolution du segment ⇒ timeout.
-- L'appartenance par segment est maintenant AGRÉGÉE une fois (`cseg_hits`,
-- `vseg_hits` : adresse → nombre de segments cochés qui la contiennent) puis
-- JOINTE à la liste des abonnés. Le nombre d'audiences satisfaites = audiences
-- « simples » satisfaites + seg_hits + vseg_hits ; réunir exige ≥ 1, croiser
-- exige = nombre d'audiences cochées. Fonction restatée intégralement.
-- ───────────────────────────────────────────────────────────────────────────
