-- Event audience demographics — add combined event CAPACITY.
--
-- The Participants ring in the per-event view needs a denominator so the red
-- arc shows real fill, not a decorative sweep. Capacity is the combined maximum
-- of the three pillars, mirroring how a "participant" is counted (tickets ∪ VIP
-- tables ∪ guest list):
--   • tickets    ← events.max_tickets          (ticket cap; null = uncapped → 0)
--   • VIP tables ← sum(vip_tables.capacity)     (seats across the venue's tables,
--                                                only when the event sells tables)
--   • guest list ← sum(guest_lists.quota)       (active lists for the event)
--
-- Only computed for the single-event view (p_event_id not null); the aggregate
-- view has no single capacity, so it returns null and the UI keeps a decorative
-- ring. nullif(...,0) means "no capacity configured" also returns null.
--
-- Everything else (scope, authorization, demographics) is unchanged from
-- 20260621190000_event_audience_demographics.sql.
