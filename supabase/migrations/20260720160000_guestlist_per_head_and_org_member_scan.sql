-- ============================================================================
-- 1. Nouveau helper : membre accepte d'une organisation.
--    Equivalent de is_venue_staff cote organisateur. Volontairement sans
--    permission fine : celui qui tient la porte d'une soiree d'organisateur
--    doit pouvoir enregistrer une entree, comme le personnel d'un club.
-- ============================================================================
CREATE OR REPLACE FUNCTION public.is_org_member(_user_id uuid, _organizer_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $fn$
  SELECT _organizer_user_id IS NOT NULL AND _user_id IS NOT NULL AND EXISTS (
...
-- ============================================================================
-- 2. record_promoter_conversion : corps repris TEL QUEL de 20260717130000
--    (extrait par script, pas retape), avec exactement deux changements.
--
-- (a) GARDE D'AUTORISATION. Elle listait le club, son personnel, le club
--     partenaire, l'organisateur et l'organisateur partenaire, mais pas les
--     MEMBRES de l'organisation. Un membre d'equipe faisant le check-in a la
--     porte declenchait « Not authorized to record this conversion » ; l'appelant
--     ne fait que journaliser l'echec, donc l'invite etait marque entre et la
--     commission perdue en silence, sans rattrapage possible.
--
-- (b) COMMISSION GUEST LIST PAR TETE. Le type 'guestlist' passait par la regle
--     'ticket'. En pourcentage, la commission valait ROUND(0 * x%) = 0 : une
--     entree gratuite n'a pas de montant. Tout promoteur sur un modele en
--     pourcentage gagnait donc exactement 0 par invite, sans que rien ne le
--     distingue d'un modele non configure. La guest list se remunere desormais
--     au forfait par tete, via rules.guestlist.value (en euros).
--
--     Les fenetres horaires restent prioritaires et inchangees : c'est deja un
--     montant fixe par tete (« 5 EUR avant 00h30, puis 2 EUR »).
-- ============================================================================
    ...
    ELSIF v_windows IS NOT NULL AND jsonb_array_length(v_windows) > 0
      ...
      IF NOT v_window_matched THEN
        IF p_conversion_type = 'guestlist' THEN
          ...
          v_commission := COALESCE((v_rules->'guestlist'->>'value')::numeric, 0);
        ELSE
