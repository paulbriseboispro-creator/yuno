...
function EventCard({
  ...
  const handleClick = () => {
    if (isSoldOut) return;
    if (ctaHref) {
      // Clic compté seulement quand on part vers la billetterie : sans lien,
      // la fiche de la soirée comptera elle-même le vrai clic (sinon doublé).
      trackAffiliateClick({
        affiliateId,
...
export default function PromoterLinktree() {
  ...
  const isByDay = effectiveSortMode === 'by_day' || !effectiveSortMode;
