    ...
    >
      ...
      <div style={{
        ...
        {(venue.logoUrl || venue.coverUrl) && (
          <img
            src={getOptimizedImageUrl(venue.logoUrl || venue.coverUrl!, { width: 148, height: 148 })}
            alt={venue.name}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: D.muted }}>
          <MapPin size={13} strokeWidth={2} color={D.muted} />
          <span style={{ fontSize: 13 }}>{venue.city}</span>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div style={{
        ...
        {org.logoUrl ? (
          <img
            src={getOptimizedImageUrl(org.logoUrl, { width: 112, height: 112 })}
            alt={org.name}
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
          />
        ) : (
          <Users size={22} color="rgba(255,255,255,.7)" strokeWidth={1.8} />
        )}
      </div>

      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: '-.01em', lineHeight: 1.15, marginBottom: 5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {org.name}
        </div>
        {org.musicGenres.length > 0 && (
          <div style={{ fontFamily: 'var(--yuno-mono, ui-monospace, monospace)', fontSize: 11.5, color: D.muted }}>
            {org.musicGenres.slice(0, 3).join(' · ')}
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '12px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 5 }}>
        {/* Venue name */}
        {event.venueName && (
          <div style={{ fontFamily: 'var(--yuno-mono, ui-monospace, monospace)', fontSize: 10, letterSpacing: '.05em', color: D.faint }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <span style={{ fontFamily: 'var(--yuno-mono, ui-monospace, monospace)', fontSize: 11, color: D.muted }}>
    ...
    const fetchFavoriteData = async () => {
      ...
      try {
        const [venueResult, affiliateVenueResult] = await Promise.all([
          clubFavs.length > 0
            ? supabase.from('venues').select('id, name, city, logo_url, cover_url').in('id', clubFavs.map(f => f.venueId).filter(Boolean) as string[])
        ...
        if (user) {
          ...
          if (followedData && followedData.length > 0) {
            const orgUserIds = followedData.map((f: any) => f.organizer_user_id);
            const { data: orgData } = await supabase.from('organizer_profiles').select('user_id, display_name, avatar_url, slug').in('user_id', orgUserIds);
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{
              ...
              <Heart size={17} strokeWidth={2.2} fill={D.red} color={D.red} />
            </div>
