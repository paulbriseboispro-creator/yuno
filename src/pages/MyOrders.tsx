        ...
        <DialogContent className="max-w-md">
            ...
            {editItems.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-background rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <Button size="icon" variant="outline" onClick={() => updateItemQty(item.id, -1)} disabled={item.qty <= 1}>
                    <Minus className="h-4 w-4" />
                  </Button>
                  <span className="w-8 text-center font-semibold">{item.qty}</span>
                  <Button size="icon" variant="outline" onClick={() => updateItemQty(item.id, 1)}>
          ...
          <div className="flex items-center justify-between pt-3 border-t">
            ...
            <span className="text-2xl font-bold text-accent">
              {editItems.reduce((sum, item) => sum + (item.qty * item.unitPrice), 0).toFixed(2)}€
            </span>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingOrder(null)}>{t('common.cancel')}</Button>
      ...
      <AlertDialog open={!!ticketToCancel} onOpenChange={() => setTicketToCancel(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {ticketToCancel && t('tickets.cancelConfirmDesc').replace('{amount}', calculateRefundAmount(ticketToCancel).toFixed(2))}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={cancellingTicket}>{t('common.cancel')}</AlertDialogCancel>
            <AlertDialogAction 
              onClick={handleCancelTicket} 
              disabled={cancellingTicket}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {cancellingTicket ? '...' : t('tickets.cancelTicket')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
