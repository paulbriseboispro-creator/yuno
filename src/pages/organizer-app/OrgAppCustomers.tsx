...
import {
  Users, Search, Ban, CheckCircle, Ticket, Table, AlertTriangle, Euro,
...
// Mirror of the owner VenueCustomer shape. `id` is the lowercased email
// (organizers have no venue_customers surrogate key). Drinks are never sold by
// organizers, so order_count is always 0.
...
interface SegmentFilters {
  ...
  value: Tier | ''; category: 'tickets' | 'tables' | ''; churn: boolean;
  origin: string;
}
...
export default function OrgAppCustomers() {
  ...
  const [incidentEmails, setIncidentEmails] = useState<Set<string>>(new Set());
  ...
  const warnedCustomers = useMemo<OrgCustomer[]>(() => {
    const accountRows = allCustomers.filter(c => c.is_banned || incidentEmails.has(c.email.toLowerCase()));
    const accountEmails = new Set(accountRows.map(c => c.email.toLowerCase()));
    const emailOnly: OrgCustomer[] = emailBans
      .filter(b => !accountEmails.has((b.email || '').toLowerCase()))
      .map(b => ({
        id: b.email, user_id: null, email: b.email, first_name: null, last_name: null, phone: null,
  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (category) list = list.filter(c => category === 'tickets' ? (c.ticket_count || 0) > 0 : (c.table_count || 0) > 0);
  ...
  const exportCustomersCsv = () => {
    const sep = ',';
    const rows = [['Email', t('profile.firstName'), t('profile.lastName'), 'Phone', 'Segment', 'Tier', 'Tickets', 'Tables', 'TotalSpent', 'Revenue30d', 'VisitsPerMonth', 'LastActivity', 'Status'].join(sep)];
    filteredCustomers.forEach(c => {
      ...
      rows.push([c.email, c.first_name || '', c.last_name || '', c.phone || '',
        ...
        String(c.ticket_count || 0), String(c.table_count || 0),
  ...
  const openCustomer = useCallback((customer: OrgCustomer) => {
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                ...
                <div className="mt-3 p-4 rounded-xl space-y-4" style={{ background: INNER_BG, border: `1px solid ${BORDER}` }}>
                  ...
                  {[
                    ...
                    { key: 'category', label: t('owner.cust.purchaseType'), opts: [{ v: 'tickets', l: t('owner.cust.tickets') }, { v: 'tables', l: t('customers.tables') }] },
                  ].map(({ key, label, opts }) => (
                    <div key={key}>
                        ...
                        {incidents.map(incident => {
                          ...
                          return (
                            <div key={incident.id} className="p-3 rounded-xl" style={{ background: INNER_BG, border: `1px solid ${F_BORDER}` }}>
                              ...
                              <p style={{ color: T2, fontSize: 12 }}>{incident.reason}</p>
      ...
      <Dialog open={showBanDialog} onOpenChange={setShowBanDialog}>
        <DialogContent className="border-0 p-0" style={{ background: '#0a0a0c', border: `1px solid ${BORDER}`, borderRadius: 18, maxWidth: 420 }}>
          ...
          <div className="p-6 space-y-4">
            ...
            <p style={{ color: T3, fontSize: 11.5 }}>{t('owner.cust.orgBanScopeNote')}</p>
