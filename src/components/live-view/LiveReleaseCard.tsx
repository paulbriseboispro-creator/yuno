...
export function LiveReleaseCard({ release, language, t, compact = false }: {
  ...
  return (
    <div
      ...
      style={{
        background: compact
          ? 'linear-gradient(180deg,rgb(var(--sheen)/.06) 0%,rgb(var(--sheen)/.02) 100%),rgb(var(--glass-10-10-12)/0.82)'
          : LV.innerBg,
    ...
    >
      ...
      <div className="mt-3" aria-label={t('lv.release.sparkline')} role="img">
        <svg viewBox="0 0 120 32" preserveAspectRatio="none" width="100%" height={compact ? 28 : 36} style={{ display: 'block' }}>
          <line x1="0" y1="31.5" x2="120" y2="31.5" stroke="rgb(var(--ink)/0.08)" strokeWidth="0.5" />
          {series.map((n, i) => {
            ...
            return (
              <rect
                ...
                height={h}
                rx={0.5}
                fill={n > 0 ? (isLast ? LV.red : LV.cMid) : 'rgb(var(--ink)/0.08)'}
