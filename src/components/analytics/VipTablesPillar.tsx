          ...
          <div key={r.label} className="space-y-1.5">
            ...
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: FAINT }}>
              <div className="h-full rounded-full transition-all"
                style={{ width: `${Math.max(r.count === 0 ? 0 : 4, Math.round((r.count / max) * 100))}%`, background: i === 0 ? RED : 'rgb(var(--ink)/var(--ink-a42,0.42))' }} />
            </div>
          </div>
        ))}
      </div>
            ...
            {(() => {
              ...
              return res.by_zone.slice(0, 8).map((z, i) => (
                <div key={z.zone_id ?? z.zone_name} className="space-y-1.5">
                  ...
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: FAINT }}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${Math.max(4, Math.round((z.revenue / zmax) * 100))}%`, background: i === 0 ? RED : 'rgb(var(--ink)/var(--ink-a42,0.42))' }} />
                  </div>
                </div>
              ));
            })()}
            ...
            {(() => {
              ...
              return tableAnalytics.reservationsByEvent.slice(0, 6).map((e, i) => (
                <div key={e.eventTitle} className="space-y-1.5">
                  ...
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: FAINT }}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${Math.max(4, Math.round((e.revenue / emax) * 100))}%`, background: i === 0 ? RED : 'rgb(var(--ink)/var(--ink-a42,0.42))' }} />
                  </div>
                </div>
              ));
            })()}
