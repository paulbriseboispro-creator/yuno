...
function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl px-3.5 py-3" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${BORDER}` }}>
...
function Chips<K extends string>({ value, options, onChange }: {
  ...
  return (
    <div className="inline-flex gap-1 p-1 rounded-xl flex-wrap" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${BORDER}` }}>
      {options.map(o => (
        <button key={o.key} type="button" onClick={() => onChange(o.key)}
          className="px-2.5 py-1 rounded-lg text-[12px] font-medium cursor-pointer transition-all duration-150"
          style={value === o.key ? { color: '#fff', background: 'rgba(255,255,255,0.12)' } : { color: T3 }}>
          {o.label}
        </button>
      ))}
    </div>
  ...
  if (loading && !data) {
    return (
      <div className="space-y-3">
        {[0, 1, 2].map(i => (
          <div key={i} className="h-36 rounded-[18px] animate-pulse" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${BORDER}` }} />
        ))}
      </div>
    );
  }
            ...
            <div className="space-y-3">
              {d.loyalty.frequency.map(fr => (
                <BarRow key={fr.bucket} label={t(`pb.who.f.${fr.bucket}`)} value={num(fr.buyers)} share={ratio(fr.buyers, freqTotal)}
                  color={fr.bucket === '1' ? 'rgba(255,255,255,0.35)' : POS} note={pct(ratio(fr.buyers, freqTotal))} />
              ))}
            </div>
          )}
        </Card>
          ...
          <Card icon={<MousePointerClick className="w-4 h-4" />} title={t('pb.conv.funnel')} sub={fill('pb.conv.funnelSub', pct(ratio(f.orders, f.sessions), 1))}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                ...
                <div key={step.k} className="rounded-xl px-3.5 py-3" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${BORDER}` }}>
