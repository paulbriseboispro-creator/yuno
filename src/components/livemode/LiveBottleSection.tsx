...
export function LiveBottleSection() {
  ...
  const handleTap = (bottle: BottleItem) => {
          ...
          >
            ...
            <div className="p-3">
              ...
              <div className="mt-1 flex items-baseline justify-between gap-2">
                <span className="font-mono uppercase" style={{ fontSize: 9.5, letterSpacing: '0.06em', color: '#5A5A5E' }}>
                  {[bottle.brand, bottle.volume_cl ? `${bottle.volume_cl}cl` : null].filter(Boolean).join(' · ')}
                </span>
                <span className="font-mono font-bold text-white" style={{ fontSize: 13 }}>
                  {Number(bottle.price)}€
                </span>
              </div>
            </div>
      ...
      <MixerSuggestionDialog
        ...
        onConfirm={(selected) => {
          if (pendingBottle) {
            addBottle(
              pendingBottle,
              selected.map((m) => ({ id: m.id, name: m.name, price: Number(m.price) }))
            );
          }
