...
export function LiveOrderPipeline({ pipeline }: Props) {
  ...
  const stages = [
    { key: 'pending',   label: t('live.pipelinePending'),   count: pipeline.pending,   color: 'var(--acc-fcd34d)',            icon: Clock },
    { key: 'paid',      label: t('live.pipelinePaid'),      count: pipeline.paid,      color: 'rgba(167,139,250,1)', icon: CreditCard },
    { key: 'preparing', label: t('live.pipelinePreparing'), count: pipeline.preparing, color: 'var(--acc-fb923c)',            icon: Flame },
  ...
  return (
    <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 18, boxShadow: CARD_SHADOW, padding: '20px 22px', overflow: 'hidden' }}>
      ...
      {/* Visual pipeline bar */}
      {total > 0 && (
        <div className="h-2 rounded-full overflow-hidden flex mb-5" style={{ background: 'rgb(var(--ink)/0.04)' }}>
          {stages.map(stage => {
