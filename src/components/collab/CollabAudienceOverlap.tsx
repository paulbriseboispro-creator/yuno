...
export function CollabAudienceOverlap({ eventId }: { eventId: string }) {
  ...
  return (
    <div className="rounded-2xl p-5"
      style={{ background: 'linear-gradient(180deg,rgb(var(--sheen)/.045) 0%,rgb(var(--sheen)/.008) 100%),var(--sf-0a0a0c)', border: '1px solid rgb(var(--ink)/0.085)' }}>
      <div className="flex items-baseline gap-2 flex-wrap">
        <span className="text-[32px] font-[700] tabular-nums leading-none" style={{ color: 'rgb(var(--ink))' }}>{nf(data.shared ?? 0)}</span>
        <span className="text-[14px]" style={{ color: 'rgb(var(--ink)/var(--ink-a60,0.6))' }}>{t('abonnés en commun', 'shared subscribers', 'suscriptores en común')}</span>
      </div>
      <p className="text-[12px] mt-1.5" style={{ color: 'rgb(var(--ink)/var(--ink-a36,0.36))' }}>
      ...
      <div className="mt-4 pt-4 grid grid-cols-2 gap-3" style={{ borderTop: '1px solid rgb(var(--ink)/0.085)' }}>
        <div>
          <div className="text-[22px] font-[680] tabular-nums" style={{ color: 'var(--acc-34d399)' }}>+{nf(data.net_new_for_venue ?? 0)}</div>
          <div className="text-[11.5px] mt-0.5" style={{ color: 'rgb(var(--ink)/var(--ink-a36,0.36))' }}>
            {t(`nouveaux pour ${venueName}`, `new for ${venueName}`, `nuevos para ${venueName}`)}
          </div>
        </div>
        <div>
          <div className="text-[22px] font-[680] tabular-nums" style={{ color: 'var(--acc-34d399)' }}>+{nf(data.net_new_for_organizer ?? 0)}</div>
          <div className="text-[11.5px] mt-0.5" style={{ color: 'rgb(var(--ink)/var(--ink-a36,0.36))' }}>
            {t(`nouveaux pour ${orgName}`, `new for ${orgName}`, `nuevos para ${orgName}`)}
          </div>
        </div>
      </div>
      <p className="text-[11px] mt-3 leading-relaxed" style={{ color: 'rgb(var(--ink)/var(--ink-a36,0.36))' }}>
