...
import { Globe, Users, MapPin, Plane } from 'lucide-react';
...
export function OwnerCustomerOrigins({ customers, onSelectCountry }: Props) {
  ...
  const { stats, byNumeric, maxCount, totalKnown, unknown, totalRevenueKnown } = useMemo(() => {
    ...
    const totalRevenueKnown = stats.reduce((s, x) => s + x.revenue, 0);
    return { stats, byNumeric, maxCount, totalKnown, unknown, totalRevenueKnown };
  }, [customers]);
