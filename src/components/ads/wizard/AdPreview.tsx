...
const srcOf = (m?: DraftMedia | null) => m?.preview || m?.url || '';

/** Définis HORS du composant : un composant recréé à chaque rendu remonterait son <img> à chaque frappe. */
function AvatarView({ pageId, size }: { pageId?: string | null; size: number }) {
  return (
...
function MediaView({ m, fill, label }: { m: DraftMedia | null | undefined; fill?: boolean; label: string }) {
  if (!m) return <div className="h-full w-full flex flex-col items-center justify-center gap-2" style={{ color: T3 }}><ImageIcon className="w-7 h-7" /><p style={{ fontSize: 12 }}>{label}</p></div>;
  if (m.kind === 'video') return <video key={srcOf(m)} src={srcOf(m)} poster={m.thumbnail_url ?? undefined} muted playsInline autoPlay loop className={`h-full w-full ${fill ? 'object-cover' : 'object-contain'} bg-black`} />;
  return <img key={srcOf(m)} src={srcOf(m)} alt="" className="h-full w-full object-cover" draggable={false} />;
}
...
export function AdPreview({ creative, pageId, pageName, igUsername, instagramOn, tab: tabProp, onTab, metaPreview, t }: {
  ...
  const Avatar = ({ size }: { size: number }) => <AvatarView pageId={pageId} size={size} />;
  const Media = ({ m, fill }: { m: DraftMedia | null | undefined; fill?: boolean }) => <MediaView m={m} fill={fill} label={t('ads.w.preview.noMedia')} />;
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: '#111113', border: `1px solid ${BORDER}` }}>
          ...
          <div className="relative" style={{ background: 'rgba(255,255,255,0.05)' }}>
            ...
            {creative.format === 'carousel' && media.length > 1 && (
              <>
                <button type="button" aria-label="previous" onClick={(e) => { e.preventDefault(); e.stopPropagation(); goCard(-1); }} className="absolute left-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full flex items-center justify-center cursor-pointer z-10" style={{ background: 'rgba(0,0,0,0.6)', color: '#fff' }}><ChevronLeft className="w-5 h-5" /></button>
                <button type="button" aria-label="next" onClick={(e) => { e.preventDefault(); e.stopPropagation(); goCard(1); }} className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full flex items-center justify-center cursor-pointer z-10" style={{ background: 'rgba(0,0,0,0.6)', color: '#fff' }}><ChevronRight className="w-5 h-5" /></button>
                <div className="absolute bottom-2 left-0 right-0 flex justify-center gap-2 z-10">
                  {media.map((m, i) => <button key={m.localId} type="button" aria-label={`${i + 1}`} onClick={() => setCard(i)} className="h-4 w-4 flex items-center justify-center cursor-pointer"><span className="h-1.5 w-1.5 rounded-full" style={{ background: i === card ? '#fff' : 'rgba(255,255,255,0.4)' }} /></button>)}
                </div>
                <span className="absolute top-2 right-2 px-1.5 py-0.5 rounded text-[10.5px] font-bold tabular-nums" style={{ background: 'rgba(0,0,0,0.6)', color: '#fff' }}>{card + 1}/{media.length}</span>
              </>
            )}
          </div>
